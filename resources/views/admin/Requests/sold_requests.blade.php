                ...
                <tr>
                    ...
                    <td>
                        <div class="item-container">
                            ...
                            <a href="#" class="item-details text-primary" data-toggle="modal"
                        ...
                        @if ($request->associatedPound)
                            <div id="poundDetails_{{ $request->id }}" class="collapse mt-2">
                                <div class="card card-body">
                                    <p><strong>Pound Serial:</strong>
                                        {{ $request->associatedPound->item_serial_number }}</p>
                                    <p><strong>Weight:</strong> {{ $request->associatedPound->weight }}g</p>
                                    <p><strong>Purity:</strong> {{ $request->associatedPound->purity }}K</p>
                                    <p><strong>Price:</strong> {{ $request->associatedPound->price }}
                                ...
                                <div class="modal-body d-flex justify-content-between">
                                    @if ($request->item_type === 'pound')
                                        ...
                                        <div class="col-md-5">
                                            ...
                                            <p><strong>Price:</strong> {{ $request->price }}
                                                {{ config('app.currency') }}</p>
                                            <p><strong>Price/Gram:</strong> {{ $pricePerGram }}
                                                {{ config('app.currency') }}g</p>
                                        </div>
                                        {{-- <div class="col-md-6">
                                    ...
                                    @else
                                        ...
                                        <div class="col-md-5">
                                            ...
                                            <p><strong>Serial Number:</strong>
                                                {{ $request->goldItem->serial_number ?? 'N/A' }}</p>
                                            <p><strong>Kind:</strong> {{ $request->goldItem->kind ?? 'N/A' }}</p>
                                            <p><strong>Model:</strong> {{ $request->goldItem->model ?? 'N/A' }}</p>
                                            <p><strong>Gold Color:</strong>
                                                {{ $request->goldItem->gold_color ?? 'N/A' }}</p>
                                            <p><strong>Weight:</strong> {{ $request->goldItem->weight ?? 'N/A' }}g</p>
                                            <p><strong>Metal Purity:</strong>
                                                {{ $request->goldItem->metal_purity ?? 'N/A' }}</p>
                                            <p><strong>Price:</strong> {{ $request->price }}
                                                {{ config('app.currency') }}</p>
                                            <p><strong>Price/Gram:</strong> {{ $pricePerGram }}
                                                {{ config('app.currency') }}/g</p>
                                        ...
                                        @php
                                            ...
                                            if (
                                                $request->goldItem &&
                                                in_array($request->goldItem->model, [
                                                    '5-1416',
                                                    '1-1068',
                                                    '5-1338-C',
                                                    '2-1928',
                                                ...
                                                $poundRequest = \App\Models\SaleRequest::where(
                                                    'item_serial_number',
                                                    $request->item_serial_number,
                                                )
                                        ...
                                        @if ($poundRequest)
                                            <div class="col-md-6">
                                                ...
                                                <p><strong>Price:</strong> {{ $poundRequest->price }}
                                                    {{ config('app.currency') }}</p>
                                    ...
                                    @if ($request->customer)
                                        <div class="row mt-3">
                                            <div class="col-6">
                                                ...
                                                <p><strong>Name:</strong> {{ $request->customer->first_name }}
                                                    {{ $request->customer->last_name }}</p>
                                                <p><strong>Phone:</strong>
                                                    {{ $request->customer->phone_number ?? 'N/A' }}</p>
    ...
    $(document).ready(function() {
        ...
        $('#filterForm').on('submit', function(e) {
            ...
            window.location.href =
                `${window.location.pathname}?filter_date=${filterDate}&status=${status}`;
        });
